...
1. **Nothing is learnt from an afternoon that happened**, which is `§6`'s last item and is untouched. `begun_at` is now kept, which is the first thing that survives an ending; the question of what else may is the one where a record of what happened turns into a verdict about a person if it is taken carelessly. Decided since, 27 August 2026: nothing else may. The reading of a page is read, acted on and dropped — `shared/vision_contracts.WhatCameBack` refuses to be pickled, copied or cached — and the durable memory grows only from what was made and what was configured. Being too fixed is the cost, and it is accepted knowingly. ⚠️ **The cost was paid and then refused.** «Nothing else may» lasted a day: `panel/what_happened.py` arrived on 28 August, and on 4 September a profile drawn from those rows was allowed too. What did not move is that the reading of a page is still not kept.
...
## 19. Il tetto al giorno, e una regola scritta che è stata ribaltata — 5 settembre 2026

Fino a stamattina `panel/rhythm.py` conteneva questa frase: «There is no setting for how many afternoons happen, and there will not be one. The days say when it may happen, the band says until when, and nothing counts what did.» Il genitore ha chiesto il contrario, e la frase è stata riscritta invece che aggirata.

**L'argomento che la reggeva era buono e si applicava a un'altra cosa.** Contare i pomeriggi che qualcuno ha fatto, e tenerne il registro, è una misura su una persona: è la macchina della ritenzione, ed è quello che `ENGAGEMENT` in `tests/test_boundaries.py` vieta ovunque. Ma un tetto al giorno non è quel conto. Quello che si tiene è **una data e un numero**, in casa e non nel cloud, azzerato a mezzanotte, letto da una sola decisione — se ne può cominciare un altro — e da nient'altro. Non arriva a un display, a un foglio, a una pagina del pannello né a un modello. Nessuna riga di storia sopravvive alla notte, quindi non c'è niente da cui si possa ricavare com'è andata una settimana.

**Quello che il vincolo vero proteggeva non era coperto da niente.** I giorni dicono quando può cominciare e la banda dice fino a quando: fra i due non c'era nessun limite. Una casa che finisce alle tre ne comincia un altro alle quattro, e un altro ancora, finché la banda regge. Con la banda 12:00–19:30 e pomeriggi da cento minuti sono quattro. Nessuno l'aveva scelto: era il residuo di una regola tolta il 25 agosto, quando il timbro «una cosa al giorno» smise di fermare i pomeriggi e non fu sostituito da niente.

**Due è il default, e non uno.** Uno è una regola, non un ritmo, e soprattutto rende il pomeriggio andato a male il pomeriggio di tutta la giornata — se la stampante è spenta, come è successo stamattina, il primo tentativo si mangia il giorno. Zero è ammesso e ferma tutto **senza togliere i giorni scelti**, che è quello a cui un genitore arriva quando la settimana è piena e non finita. Sei è il fondo scala: sta sopra quello che l'orologio può raggiungere, quindi non modella niente. È la stessa correzione registrata al §12 di `ideas/11` e per `MAX_ILLUSTRATION` — un tetto che viene colpito spesso non è un tetto.

**La pressione del tasto non lo scavalca, ed è la decisione discutibile.** «Fai cominciare un'attività adesso» scavalca il giorno e l'ora d'inizio; non ha mai scavalcato la fine della banda, e ora non scavalca nemmeno questo numero. La ragione è che chi preme è lo stesso che ha scritto il numero, e alzarlo è un campo: un tasto che passa sopra il tetto lo rende una decorazione. `tests/test_begin_now.py` conteneva il test opposto — «lo stampo non è ciò che una pressione deve superare» — ed è stato riscritto dicendo perché si è girato, non cancellato. Una pressione rifiutata dal tetto viene **cancellata** e non lasciata in piedi: sopravviverebbe alla mezzanotte, quando il conto torna a zero e l'ora che scavalca è il cuore della notte.

**Un guasto trovato mentre lo si scriveva.** `_to_rhythm` in `panel/cosmos_store.py` leggeva `int(document.get("scriptsWanted") or DEFAULT_SCRIPTS_WANTED)`: uno zero salvato è falsy, quindi un genitore che aveva chiesto «nessuna idea in attesa» si ritrovava dieci. Valeva già prima di questo lavoro e valeva per un campo il cui minimo è zero. Corretto con `_whole`, che distingue assente da scelto, e usato anche dal campo nuovo.

**Fatto quando:** il campo è nel pannello, l'hub lo rispetta, e un giro di timer con il tetto raggiunto scrive in chiaro quanti ne sono cominciati e quanti ne erano ammessi.
