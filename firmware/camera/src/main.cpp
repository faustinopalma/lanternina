...
static constexpr int LED = 6;
static constexpr gpio_num_t RETIRED_LED_D4 = GPIO_NUM_5;
...
static void reportStatus(const char *phase) {
    if (WiFi.status() != WL_CONNECTED || time(nullptr) < 1700000000) return;
    {
        ...
        if (http.begin(tls, String(HUB_URL) + "/status")) {
            ...
            status["firmware"] = "camera-2026-09-14-d5-d8";
...
void setup() {
    rtc_gpio_pullup_dis(RETIRED_LED_D4);
    rtc_gpio_pulldown_dis(RETIRED_LED_D4);
    rtc_gpio_deinit(RETIRED_LED_D4);
    pinMode(RETIRED_LED_D4, INPUT);
    ...
    while (Serial.available()) {
        ...
        if (character == '\n') {
            ...
            if (strcmp(command, "USB_TEST_ON") == 0 && usb) {
                ...
                uint32_t retiredD4Mux = REG_READ(IO_MUX_GPIO5_REG);
                Serial.printf("retired_gpio=5 pullup=%d pulldown=%d input_enabled=%d output_enabled=%d\n",
                              (retiredD4Mux & FUN_PU) != 0, (retiredD4Mux & FUN_PD) != 0,
                              (retiredD4Mux & FUN_IE) != 0,
                              (REG_READ(GPIO_ENABLE_REG) & (1UL << RETIRED_LED_D4)) != 0);
