...
The [ten authored activity references](authored-examples/README.md) add concrete activities for an adolescent with moderate intellectual disability, with explicit support assumptions. Each contains parent and script outputs, display text, page contracts, ready-to-send image prompts, hypothetical returns, continuations and alternative paths. These are development references written by the coding assistant, not model-run results or evidence of educational effectiveness. The collection includes a comparison protocol and a local contract checker; it does not change the production prompts.
