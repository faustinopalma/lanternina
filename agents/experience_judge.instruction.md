...
- **`a_beat_with_no_mark`** — a moment needs an observable result to proceed, but supplies no way to communicate it through available equipment. Do not require written marks for every activity: a choice, object photograph or another supported response may suffice.
...
- **`does_not_end_on_the_object`** — the final moment claims an outcome that was not established or leaves the activity without an explicit closure. A factual summary or explanation is allowed; grading the person is not.
