...
Do not write an id, a format version or a list of what the house needs: those are known already and are not yours to write.
  ...
  "strategy": at most $max_strategy characters, and use them. This is the game itself, written out for whoever runs the afternoon — not a summary, not a statement of intent, not a paragraph about what you hope it will feel like. Write it in these parts, in this order, each headed by its own name on its own line:
