...
| The system starts stating verdicts | Adaptation needs a model of what happened, and a model is one rename away from a claim about the person | **Changed on 4 September 2026.** A profile is now kept and updated at system level, because pitching an afternoon at the right level cannot be done without one. What did not change is where it may appear: it reaches the two models that write and run an afternoon, and it reaches no display, no sheet and no page a parent reads. A reading is still read and not kept — `WhatCameBack` closes the implicit routes out of memory — and `tests/test_boundaries.py` still refuses the vocabulary in the browser and as a field a prompt asks a model to fill in |
