...
The activity format supports `collect` with `source: camera`, requiring `photograph_table`; existing moments default to the scanner. Object collection sends one photograph and activity context. A photograph returned to a paper collection moment sends the original A4 image used for printing followed by the photograph, so the model can describe and transcribe added marks. Both photographic modes require explicit uncertainty and skip the scanner page-placement call. Uncertain readings leave the moment waiting. Capture timestamps bind a photograph to one waiting run and moment; unknown or late captures remain archived. USB diagnostic captures never advance activities. Outside an activity, only displays explicitly assigned the photo job can show camera originals. A worker interrupted after claiming a photo requires explicit review and can be archived without replaying physical effects. The multi-button response panel, photographed-handwriting acceptance and a second household installation remain open. Infrastructure and historical operational notes remain in [DEPLOY.md](DEPLOY.md), [HARDWARE.md](HARDWARE.md) and [WORKLOAD-IDENTITY.md](WORKLOAD-IDENTITY.md).