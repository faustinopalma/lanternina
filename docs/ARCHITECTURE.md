...
The activity format supports `collect` with `source: camera`, requiring `photograph_table`; existing moments default to the scanner. The photographic reader receives one image and activity context, without a blank page or the paper-placement call. Uncertain readings leave the moment waiting. Capture timestamps bind a photograph to one waiting run and moment; unknown or late captures remain archived. USB diagnostic captures never advance activities. A worker interrupted after claiming a photo requires explicit review and can be archived without replaying physical effects. The multi-button response panel, physical camera activity acceptance and a second household installation remain open. Infrastructure and historical operational notes remain in [DEPLOY.md](DEPLOY.md), [HARDWARE.md](HARDWARE.md) and [WORKLOAD-IDENTITY.md](WORKLOAD-IDENTITY.md).