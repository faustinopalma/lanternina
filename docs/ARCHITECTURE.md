...
| Raw reading contract | `shared/vision_contracts.py` | Implicit copy and pickle are refused. Explicit `to_dict()` permits the required HTTP hop and can be persisted by a caller. |
