...
**What came back is kept too, as of 28 August 2026, and this replaces the promise that it was not.** The old rule was that the reading lasted as long as the afternoon and was then gone. It made the system's own aim unreachable: an afternoon written from nothing but the settings a parent typed once cannot move, and a system that cannot move is the failure this project was most likely to reach. So a row per afternoon is kept — what it was about, how far it got, how it ended, and for each sheet whether it came back marked or blank and what the reader said was on it. `panel/what_happened.py` is the whole of it, and the next afternoon is written from it.

What did not change is the sentence above this one: none of it is a verdict. The stored shape has no field for a score, a level, an ability or a summary of a person, and `tests/test_what_happened.py` names every field it does have, so one would have to be added there before it could be written anywhere. What a model makes of these facts it makes while it writes, and it is gone again. The image and the handwriting are still not kept. The parent can read all of it and delete all of it.

What that costs, said plainly: a household's record is now a record of a person as well as of a machine. The guarantee moved from "there is nothing to keep" to "there is nowhere to put a judgement", which is weaker, and it is the reason the field list is a test.
