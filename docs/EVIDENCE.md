...
On 9 September 2026 Fausto confirmed that the assembled circuit uses the source article's D2/GPIO3 LED and D3/GPIO4 shutter. USB probing measured 8 MiB flash and 8 MiB PSRAM. Firmware with that wiring compiled and was flashed after an 8,388,608-byte backup. The [Battery Usage section](https://wiki.seeedstudio.com/xiao_esp32s3_getting_started/#battery-usage) explicitly states that this base board has no battery-to-GPIO measurement connection. The deployed inventory therefore shows USB power and an unavailable battery voltage, not a charge percentage. The optional sensing circuit has not been assembled or calibrated.
