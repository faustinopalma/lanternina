...
- **The parent does not watch it happen.** No progress view, no help log, no per-step timing, no suggested questions afterwards — the person who did it is the only source. A parent's message while something runs is a typed fact or constraint (new end hour, pause, broken device, missing material), never free text reaching a model.
