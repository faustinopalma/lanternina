...
- **One afternoon may hand over more than one sheet.** Two calm pages beat one crowded page, and a page that has to carry everything is the page nobody reads. The bound is somebody's attention and the printer's paper, not a rule: an encyclopedia every time is the other failure. `docs/EVIDENCE.md §2` has the measurements.
