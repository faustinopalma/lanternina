...
- **Say what a thing is, not what it is not.** A negative only informs a reader who expected the positive, and most do not: "there is no layout engine" tells nobody anything, while "the page is described in words and drawn in one pass" tells them what happens. Use a negative when the reader would otherwise assume the opposite — "no model runs on the device" earns its place.
- **Explain, do not defend.** Anything written for a reader outside this repository states how the thing works, in the order it happens. A decision that could have gone the other way is worth writing down in `ideas/`, where the reasoning lives; it is not a heading on a page somebody reads to find out what the system does. "Four decisions worth arguing about" is the shape of a pitch, not of an explanation.
