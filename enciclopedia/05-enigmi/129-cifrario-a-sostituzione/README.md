...
- **Fonti** [Substitution cipher](https://en.wikipedia.org/wiki/Substitution_cipher), [Pigpen cipher](https://en.wikipedia.org/wiki/Pigpen_cipher), [Nyctography](https://en.wikipedia.org/wiki/Nyctography), [Frequency analysis](https://en.wikipedia.org/wiki/Frequency_analysis), [Letter frequency](https://en.wikipedia.org/wiki/Letter_frequency), [Cipher](https://en.wikipedia.org/wiki/Cipher), tutte prese il 30 agosto 2026
...
- **Con che cosa si sostituisce.** Altre lettere, cifre, o simboli. «Substitution cipher» avverte che i simboli non aggiungono niente alla sicurezza: qualunque insieme di segni strani si può ritrascrivere in lettere e trattare come al solito. Aggiungono però moltissimo all'aspetto, e l'aspetto è quasi tutto quello che serve in casa.
...
Un limite tecnico, e questa è la voce in cui morde di più: **un modello linguistico non sa manipolare le lettere dentro le parole** (misurato). Non può cifrare, non può decifrare, e non può controllare un testo cifrato. Un messaggio cifrato da un modello sarebbe sbagliato in modo invisibile, e il danno sarebbe totale — chi lo decifra non otterrebbe un errore, otterrebbe un'assurdità, e non avrebbe modo di sapere di chi sia la colpa.

**L'unica strada era già stata trovata alla scheda 5, corrispondenza (matching): una legenda data, o meglio una legenda stampata vuota e riempita da chi risponde.** Una tabella a due colonne è una macchina, e la seconda colonna la scrive una mano. Il foglio porta la cornice e nessuna macchina tocca una lettera; l'operazione di cifratura diventa una consultazione, che è lenta ma non si sbaglia in silenzio, perché una lettera cercata nella tabella o si trova o non si trova.

## Esempi trovati

Da «Substitution cipher», con la chiave *zebras*: `flee at once. we are discovered!` diventa `SIAA ZQ LKBA. VA ZOA RFPBLUAOAR!`, e scritto a gruppi di cinque `SIAAZ QLKBA VAZOA RFPBL UAOAR`.
...
La tabella stampata vuota è l'unica forma in cui questa voce sta in casa, ed è la stessa cosa già notata alla scheda 5, corrispondenza (matching): **una legenda data — o meglio, una legenda da riempire — è il modo di usare un cifrario senza chiedere a nessuna macchina di toccare una lettera.** Qui il foglio porta ventuno righe vuote e nessuno sa che cosa ci finirà.
...
**Dove si romperebbe.** Se il foglio portasse un messaggio già cifrato da un modello, sarebbe sbagliato e nessuno se ne accorgerebbe: la tabella qui è vuota apposta. Un pannello di quattro righe non può portare la tabella — ventuno righe non ci stanno — ma può portare la sola cosa che serve la sera: *stanotte, tre righe al buio*.

## Che cosa la rende interessante

**Sostituzione e trasposizione sono le due sole operazioni su un testo,** e l'enciclopedia le ha adesso tutte e due: la voce 121, anagramma e questa. Ogni gioco di lettere dell'elenco è una delle due, o un misto: il capitolo 12 si può leggere secondo questo asse invece che per nome del gioco.

**I simboli strani comprano l'aspetto, non la segretezza,** e la fonte lo dice senza pietà del pigpen: chi lo intercetta lo legge come lo legge il destinatario. La conseguenza è liberatoria invece che deprimente — se quello che si vuole è che una cosa **sembri** un codice, un cifrario noto va benissimo, e nessuno deve rompere niente.

**Una tabella stampata vuota è una macchina, e si può stampare senza saperne il contenuto.** Vale per i cifrari, per le legende delle mappe, per le notazioni inventate. È l'unica famiglia in cui il limite di un modello sulle lettere non morde affatto, ed è già arrivata due volte per strade indipendenti — alla scheda 5, corrispondenza (matching) e alla voce 116, indovinello dell'oggetto che scrive.
