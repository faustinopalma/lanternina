...
- **Fonti** [Proof without words](https://en.wikipedia.org/wiki/Proof_without_words), [Triangular number](https://en.wikipedia.org/wiki/Triangular_number), [Squared triangular number](https://en.wikipedia.org/wiki/Squared_triangular_number), lette il 2 settembre 2026

## Che cos'è

Una figura che mostra perché un'affermazione è vera, senza dire niente. «Proof without words» la definisce come «l'illustrazione di un'identità o di un enunciato matematico che si può dimostrare autoevidente per mezzo di un diagramma, senza nessun testo esplicativo di accompagnamento».
...
**Che cosa chiude, in numeri.** L'identità che la nostra figura mostra — la somma dei primi *n* numeri dispari fa *n*² — vale per ogni *n*, cioè per un insieme infinito di casi, e la figura ne disegna uno. Verificata per formula fino a *n* = 400 e per una seconda strada fino a 40: si costruisce il quadrato *n*×*n* e si conta quante caselle stanno in ogni squadra a L, ottenendo 1, 3, 5, 7, … Le due strade non sono la stessa scritta due volte — una somma i dispari, l'altra conta le caselle di una griglia — e concordano. Il teorema di Nicomaco è verificato allo stesso modo fino a *n* = 300.
...
**Un aneddoto di scuola è falso, e l'ha smontato una fonte che non se ne occupava.** La storia di Gauss bambino è definita apocrifa da «Triangular number», che aggiunge una data — Dicuil, 816 — e un'ipotesi più antica, i pitagorici. Serve qui perché quella storia è il modo più diffuso di raccontare esattamente questa identità.

## Esempi trovati

Da «Proof without words», la somma dei dispari: in un angolo di una griglia un quadretto vale 1; lo si circonda su due lati con una striscia di tre, e viene un blocco 2×2, cioè 4; altri cinque e viene 3×3, cioè 9. E così avanti.
...
Da «Triangular number»: la dimostrazione senza parole che ogni numero esagonale è un numero triangolare di lato dispari. E il problema delle strette di mano, la cui risposta per *n* persone è il triangolare di *n*−1.

Da «Squared triangular number»: la tavola pitagorica di Row, 1893, sommata per righe — ogni riga è *i* volte un numero triangolare — e sommata per gnomoni annidati, dove ogni gnomone fa un cubo.
...
**Una figura è una dimostrazione solo se si vede come continuare.** La condizione della fonte — generalizzabile — è anche il criterio di disegno: la nostra scheda la soddisfa chiedendo il caso successivo. Ogni volta che si consegna un esempio invece di una regola, la domanda da porsi è la stessa.
