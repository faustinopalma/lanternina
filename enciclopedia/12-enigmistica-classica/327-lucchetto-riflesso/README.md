...
- **Fonti** [Lucchetto (enigmistica)](https://it.wikipedia.org/wiki/Lucchetto_(enigmistica)), [Biscarto](https://it.wikipedia.org/wiki/Biscarto), [Bifronte](https://it.wikipedia.org/wiki/Bifronte), prese il 1 settembre 2026

## Che cos'è

Un lucchetto in cui la chiave, nella seconda parola, è scritta al rovescio. «Lucchetto (enigmistica)» lo scrive **XZz / zZY = XY** e porta *spia / aiola = spola*: in fondo alla prima c'è *ia*, in testa alla seconda c'è *ai*, e sparisce da tutte e due.
...
«Lucchetto (enigmistica)», presa il 1 settembre 2026, lo dà come «un tipo particolare di lucchetto» e non gli assegna né una data né un autore proprio: eredita quelli del lucchetto, teorizzato nel 1950 da Pietro Mercatanti, che firmava Carminetta.

Sta quindi in una famiglia con una genealogia scritta — lucchetto 1950, cerniera 1955, biscarto 1963, doppia estrazione 1973, cernita 1975 («Biscarto») — e ne è l'unico membro che non compare in quell'elenco di date, perché non è un gioco nuovo ma una variante di lettura.
...
- **Bifronte** (333) — l'altra forma del capitolo che chiede di leggere al contrario, ma là si rovescia la parola intera e non un pezzo («Bifronte»).
...
Controllato su tutte le coppie di stringhe fino a quattro lettere su un alfabeto di tre: 1 296 coppie hanno una chiave lunga almeno due, e nessuna di queste è senza la chiave corta. Vale anche per i due esempi della fonte: *spia / aiola* ammette *ia* e *a*, *torre / erba* ammette *re* e *e*, e nei due casi corti viene *spiiola* e *torrrba*.
...
Da «Lucchetto (enigmistica)», riscritti: *spia / aiola = spola*; *torre / erba = torba*.

La fonte non ne dà altri, e non dà nessuna misura: «Lucchetto (enigmistica)» è una pagina di poco meno di tre kilobyte che descrive il lucchetto e nomina la variante riflessa in una frase.
...
Dove si romperebbe: un modello linguistico non sa manipolare le lettere dentro le parole (misurato), quindi non può costruire le cinque coppie. Le abbiamo cercate a mano e verificate con un programma.

## Che cosa la rende interessante

**Una variante può togliere il controllo dell'errore che la forma di partenza aveva.** Il lucchetto della voce 326, lucchetto si verifica nel materiale; rovesciare la chiave lo riporta nel vocabolario, e per una ragione strutturale — la chiave corta è sempre disponibile. Quando una forma sta nel formato, le sue varianti non ci stanno per eredità, e vanno guardate una per una.
