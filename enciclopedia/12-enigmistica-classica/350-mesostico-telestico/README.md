...
- **Fonti** [Mesostico](https://it.wikipedia.org/wiki/Mesostico), [Telestico](https://it.wikipedia.org/wiki/Telestico), [Mesostic](https://en.wikipedia.org/wiki/Mesostic), [Gioco enigmistico](https://it.wikipedia.org/wiki/Gioco_enigmistico), prese il 1 settembre 2026. [Mesostico](https://it.wikipedia.org/wiki/Mesostico) dichiara di essere un abbozzo, ed è di 1 455 byte: la definizione e i rimandi, e nient'altro
...
Il **telestico** prende l'ultima. «Telestico», presa il 1 settembre 2026: «con procedura inversa rispetto all'acrostico, le lettere o le sillabe o le parole finali di ciascun verso formano un nome o una frase». Il **mesostico** prende una lettera in mezzo. «Mesostico», presa lo stesso giorno: «sono le lettere o le sillabe o le parole **centrali** di ciascun verso, e non quelle iniziali, che formano un nome o una frase».
...
- **Se si fanno insieme.** «Telestico» scrive che il telestico «viene spesso associato» all'acrostico, e nei casi più complessi anche al mesostico.
...
- **Quadrato del Sator** — il caso in cui i procedimenti si sommano fino a leggersi in tutte le direzioni; «Telestico» lo cita esattamente per questo.
...
**Il mesostico è lo spazio di ricerca più grande di tutta la sezione, e non lo si sarebbe detto.** Contato sul mesostico che «Mesostic» riporta per esteso: se le maiuscole non ci fossero, le sue sette righe — contate in lettere, che sono le sole a poter portare il verticale — offrirebbero **4 665 600** letture, contro le 2 097 152 spartizioni del rebus della voce 346, rebus. Il conto è stato fatto per due strade, il prodotto delle lunghezze e l'enumerazione vera delle scelte sulle prime quattro righe, dove i casi sono ancora contabili. Le maiuscole lo riducono a **una**.

**Quindi il mesostico fa con la tipografia quello che il rebus fa con il diagramma.** È la stessa mossa: dichiarare accanto alla domanda una grandezza che la risposta deve avere, e che non rivela la risposta. La differenza è che il diagramma è un numero e la colonna è un modo di stampare, quindi la leva c'è su qualunque foglio.

**La regola di Culver è l'unica regola di questa sezione che una macchina possa controllare da sola.** Verificata sul mesostico *KITCHEN*: fra ogni coppia di maiuscole consecutive non compare nessuna delle due lettere, quindi è al cento per cento; e la regola al cento per cento implica quella al cinquanta, mentre il contrario non vale. Sono due tarature della stessa condizione, non due giochi. È un controllo sulla **presenza di una stringa**, cioè della stessa specie della *macrologia* che la voce 337, indovinello in versi aveva isolato come l'unico dei sette difetti dell'indovinello verificabile a macchina.
...
**Le due lingue classificano diversamente, e la freccia lo dice.** Controllato il 1 settembre 2026: in inglese `Telestich` **rimanda** ad `Acrostic`, cioè il telestico non ha una pagina propria e vive dentro l'acrostico; in italiano `Telestico` e `Mesostico` sono due voci separate. La stessa cosa è una variante da una parte e una forma dall'altra, e il rimando è l'affermazione che lo dichiara.

**Sulla generazione automatica, qui cade il confine.** La voce 349, acrostico ha lasciato un'ipotesi da verificare: che un modello linguistico sappia costruire un acrostico, perché l'unica lettera da maneggiare è la prima di una riga, cioè un confine. Il telestico sta dalla stessa parte — l'ultima lettera è l'altro confine. **Il mesostico no**: chiede la *k*-esima lettera dentro una parola, che è esattamente l'operazione che un modello linguistico sbaglia. Se l'ipotesi regge, questa voce è mezza dentro e mezza fuori, e il taglio passa in mezzo a lei. Non è stato provato, e **va verificato**.

## Esempi trovati

Dal Nordafrica, quarto secolo: l'iscrizione del *Praedium Sammacis*, otto esametri le cui iniziali danno PRAEDIVM e le cui finali danno SAMMACIS. Ricontato per due strade — riga per riga, e impaginando le otto righe in una griglia di caratteri e leggendone due colonne.
...
Da John Cage, riportato da «Mesostic»: un mesostico al cento per cento su KITCHEN, sette righe in inglese.
...
Dove si romperebbe: su un pannello di quattro righe corte l'iscrizione non ci sta, e un mesostico di quattro lettere sì. Su un foglio stampato in bianco e nero ci sta tutto, e la colonna la fa il maiuscolo, che non ha bisogno di colore.
