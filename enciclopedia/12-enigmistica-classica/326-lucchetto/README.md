...
- **Fonti** [Lucchetto (enigmistica)](https://it.wikipedia.org/wiki/Lucchetto_(enigmistica)), presa il 30 agosto 2026; [Biscarto](https://it.wikipedia.org/wiki/Biscarto), [Cerniera (enigmistica)](https://it.wikipedia.org/wiki/Cerniera_(enigmistica)), [Doppia estrazione](https://it.wikipedia.org/wiki/Doppia_estrazione), [Longest common substring](https://en.wikipedia.org/wiki/Longest_common_substring), prese il 1 settembre 2026
...
- **Cerniera** (329) — lo stesso doppio scarto con le chiavi dalla parte opposta: ZX / YZ = XY, in capo alla prima e in coda alla seconda. La prima stesura di questa scheda diceva «in mezzo a entrambe», che è invece il biscarto centrale; corretto il 1 settembre 2026 su «Cerniera (enigmistica)».
- **Doppia estrazione** — la chiave sta ai due estremi di tutte e due le parole: ZXZ / ZYZ = XY («Doppia estrazione»).
...
Fonte: «Lucchetto (enigmistica)», presa il 30 agosto 2026. **La prima stesura di questa scheda era a memoria e sbagliava due cose**: diceva che il lucchetto è uno dei giochi più antichi del repertorio (è del 1950) e dava uno schema approssimativo. La correzione è nel testo qui sopra, e vale la pena tenerla come misura di quanto la memoria sia inaffidabile proprio dove suona più sicura.
...
Fuori dalla famiglia, l'operazione «il pezzo più lungo in fondo alla prima che è anche in testa alla seconda» è quella che l'informatica usa per rimettere insieme frammenti sovrapposti. La pagina che se ne occupa, «Longest common substring», è stata presa e **non è citabile sui costi**: l'estrazione a testo perde le formule, e le complessità arrivano come «time using» senza l'espressione. Si dice e si va avanti.
...
Da «Cerniera (enigmistica)», per confronto: *vita / navi = tana*, e la stessa terna si può girare — *navi / tana = vita*, *tana / vita = navi*. **La cerniera è ciclica e il lucchetto no**, e le tre rotazioni sono state rifatte.

Da «Doppia estrazione»: *atollo / appello = toppe*, e la tripla *pirone / pitone / pilone = rotolo*.
...
Un modello linguistico non può costruire un lucchetto. Si può però fornire il materiale fisico e lasciare che il lucchetto lo faccia chi gioca.
...
Le dodici strisce sono materiale, e il materiale si corregge da sé: la sillaba combacia o no, e si vede sovrapponendo. Basta stampare dodici parole scelte perché alcune si aggancino — il che è alla portata di un modello, perché la sillaba iniziale e finale di una parola sono cose che sa dire, a differenza delle lettere in mezzo.
...
Dove si romperebbe: le cinque coppie le abbiamo cercate a mano e verificate con un programma. Un foglio nuovo ogni settimana avrebbe bisogno di un elenco di parole italiane in casa, perché un modello linguistico non sa manipolare le lettere dentro le parole (misurato).
