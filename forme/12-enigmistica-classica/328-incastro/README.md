...
- **Fonti** `it-incastro.txt` e `it-sciarada.txt`, prese il 30 agosto 2026; `it-intarsio.txt` e `riffle-shuffle-permutation.txt`, prese il 1 settembre 2026
- **Stato della ricerca** fatta, 1 settembre 2026

## Che cos'è

Si taglia in due la prima parola e ci si mette dentro la seconda, intera. `it-incastro.txt` la scrive **XX / Y = XYX**: *cane* tagliata in *ca* e *ne*, *micio* in mezzo, e viene *camicione*.
...
La glossa dell'elenco non funziona. Con *pane* e *ora* gli incastri possibili sono cinque — *opanera*, *orpanea*, *panorae*, *paorane*, *poraane* — e *panora* non è fra questi: ha sei lettere invece delle sette della coppia, cioè per strada ne perde una, e un incastro non perde niente. Verificato in `build/check_328.py`.

## Da dove viene

`it-incastro.txt`, presa il 30 agosto 2026, dà una data e un nome: le prime combinazioni si devono al **Tarlo, nel 1879**, e si chiamavano *parole incastrate*. Prima del nome attuale il gioco ne ebbe altri tre — *sciarada mista*, *innesto*, *parola avvinta* — e la sequenza dei nomi dice che per un pezzo l'incastro fu visto come una variante della sciarada e non come una forma sua.
...
- **Intarsio (tarsia)** (330) — la seconda parola entra spezzata invece che intera. È la differenza che `it-intarsio.txt` usa per definire l'intarsio.
...
`it-incastro.txt` è breve — 2 181 byte — e non contiene nessuna misura: dà la formula, la data e gli esempi.
...
Sommando: sciarada 7, incastro 21, intarsio 42, sciarada alterna 57, e 7 + 21 + 42 + 57 = 127. Verificato per formula binomiale e enumerando tutte le 128 stringhe in `build/check_328.py`. **I quattro giochi sono le quattro classi di una sola partizione**, e le pagine che li descrivono non lo dicono mai: ognuna definisce il suo per differenza dalla precedente.
...
Da `it-incastro.txt`, riscritti: *cane / micio = camicione*; e in versione a frase *casco / adagio = casa da gioco*, dove le undici lettere della coppia sono le stesse undici del totale.
...
Il limite tecnico del capitolo — il sistema non sa manipolare le lettere dentro le parole — non morde: al sistema si chiede di stampare ventun stringhe che si ricavano tagliando una parola in tutti i modi, non di giudicarle. Il giudizio «questa è una parola italiana» resta fuori dal sistema, e in casa un vocabolario non è garantito.
...
**Un gioco può essere facile nei due versi, ed è raro.** Dal totale 21 letture, dalle due parole 3: l'incastro non ha un verso difficile. Alla rassegna vale la pena chiedersi se una forma senza verso difficile sia ancora un enigma, o soltanto un esercizio di lettura.
