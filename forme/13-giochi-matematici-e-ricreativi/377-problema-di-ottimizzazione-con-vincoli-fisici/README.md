...
- **Fonti** `steiner-tree-problem.txt`, `soap-bubble.txt`, `minimal-surface.txt`, `plateaus-problem.txt`, `catenary.txt`, `isoperimetric-inequality.txt`, `it-bolla-di-sapone.txt`, `travelling-salesman.txt`, lette il 2 settembre 2026
...
**La pagina italiana sulle bolle è corretta e non aggiunge niente di misurato.** `it-bolla-di-sapone.txt` dà la ragione della forma sferica — la sfera ha la superficie minima a volume dato — e la regola della fusione di due bolle, ma non riporta né le leggi di Plateau con i loro angoli né l'uso come calcolatore. È stata letta e serve solo a confermare la parte già coperta.

## Esempi trovati

Da Frei Otto, in `soap-bubble.txt`: la geometria di un tetto teso determinata immergendo un telaio in acqua saponata. È un edificio progettato da una lamina.

Da Gaudí, in `catenary.txt`: gli archi a catenaria sotto il tetto di Casa Milà a Barcellona. Gaudí li trovava appendendo catenelle e guardando il modello capovolto.

Da `catenary.txt`, come curiosità che si può costruire: una ruota **quadrata** rotola perfettamente liscia su una strada fatta di gobbe a catenaria capovolta. Funziona per ogni poligono regolare tranne il triangolo.
...
Da `travelling-salesman.txt`: Dantzig, Fulkerson e Johnson nel 1954 usarono uno spago per esaminare i giri sulla loro istanza di 49 città. È la voce 153, problema di ottimizzazione che lo segnala, ed è il caso in cui lo strumento fisico serve a chi il conto lo sa fare.
